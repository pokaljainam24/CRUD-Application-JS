...
const latestProductRow = document.getElementById('latest-product-row');
...
function renderProducts() {
    ...
    for (let i = startIndex; i < endIndex; i++) {
        ...
        // Check if the product has multiple images and create a carousel
        if (product.images.length > 1) {
                    ...
                    <div class="carousel-inner">
                        ${product.images.map((img, index) => `
                            <div class="carousel-item ${index === 0 ? 'active' : ''}">
                                <img height=400 src="${img}" class="d-block w-100" alt="${product.title}">
                            </div>
                        `).join('')}
                    </div>
            ...
            // If only one image, display it as normal
            imageContent = `<img src="${product.images[0]}" class="card-img-top" alt="${product.title}">`;
        }
...
function buyNow(productId) {
    ...
    const productDetailsHTML = `
        ...
        <p><strong>Rating:</strong> ${generateStars(product.rating)} (${product.rating} / 5)</p>
    ...
    // Attach the "Buy Now" functionality
...
function updateCartBadge() {
    ...
    // Show or hide the cart badge based on the cart item count
    if (cartItemCount > 0) {
...
function addProductCard(product) {
    ...
    // Log product object for debugging purposes
    console.log(product);

    let imageContent = '';

    // Ensure product.name is defined or use a fallback
    const productName = product.name || product.title || "Untitled Product";

    // Check if the product has multiple images
    if (product.images && product.images.length > 1) {
        // Create a Bootstrap Carousel for multiple images
        imageContent = `
            <div id="carousel-${product.id}" class="carousel slide" data-bs-ride="carousel">
                <div class="carousel-inner">
                    ${product.images.map((img, index) => `
        ...
        // If only one image, just show the regular image
        imageContent = `<img src="${product.images[0]}" class="card-img-top" alt="${productName}">`;
    }

    productCol.innerHTML = `
      <div class="card border-0">
        ${imageContent}
        <div class="card-body text-start bg-dark" id="card-body">
          <h6 class="text-success">${product.category}</h6>
          <h5 class="card-title text-primary"><span class="text-light">Product Name : </span> ${productName}</h5>
          <h6 class="fw-bold text-info mt-1"><strong class="text-light">Description : </strong>${product.description}</h6>
          <p class="card-text text-primary"><span class="text-light fw-bold">Prise : </span>£${product.price}</p>
          <p class="text-danger fw-bold"><strong class="text-light">Discount: </strong>${product.discountPercentage}%</p> 
          <p class="text-info fw-bold"><span class="text-light">Stock: </span>${product.stock}</p>
          <p class="mb-0 text-warning"><span class="text-light fw-bold">Ratings:</span> ${generateStars(product.rating)} <span class="text-success fw-bold">${product.rating}</span></p><hr class="text-light">
          <p class="text-light mt-2"><strong>SKU:</strong> ${product.sku}</p>
                <p class="text-light"><strong>Weight:</strong> ${product.weight} kg</p>
                <p class="text-light"><strong>Warranty:</strong> ${product.warrantyInformation}</p>
                <p class="text-light"><strong>Shipping Info:</strong> ${product.shippingInformation}</p>
    ...
    latestProductRow.appendChild(productCol);  // Append to latest product section
}


function renderLatestProducts() {
    // Clear the existing content in the latest product section
    latestProductRow.innerHTML = '';

    // Skip the first 24 products and display the next 6 products
    const startIndex = 24; // Starting from the 25th product
    const latestProducts = products.slice(startIndex, startIndex + 6);  // Get the next 6 products
...
updateCartBadge();
renderProducts();  // Render the first 24 products
renderLatestProducts();  // Render the next 6 products in the Latest section
